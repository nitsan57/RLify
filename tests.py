...
def test_2e2():
    ...
    model_kwargs = {'embed_dim': 128, 'repeat':2}  # the chosen neural network args
    agent = PPO_Agent(obs_space=env.observation_space, action_space=env.action_space, device=device, batch_size=1024, max_mem_size=10**5,
                    num_parallel_envs=4, lr=3e-4, entropy_coeff=0.05, model_class=model_class, model_kwargs=model_kwargs, discount_factor=0.99, kl_div_thresh=10)
    train_stats = agent.train_n_steps(env=env,n_steps=650000)
    utils.plot_res(train_stats, 'reward', smooth_kernel=100, render_as="browser")

    mean_reward = agent.run_env(env, best_act=True, num_runs=1)
    ...
    # agent = PPO_Agent(obs_space=env.observation_space, action_space=env.action_space, device=device, batch_size=1024, max_mem_size=10**5,
    #                 num_parallel_envs=4, lr=3e-4, entropy_coeff=0.05, model_class=model_class, model_kwargs=model_kwargs, discount_factor=0.99, kl_div_thresh=0.06, clip_param=0.1)
    # train_stats = agent.train_n_steps(env=env,n_steps=250000)

    # # agent.load_highest_score_agent()
    # utils.plot_res(train_stats, 'reward_new', smooth_kernel=10, render_as="browser")
