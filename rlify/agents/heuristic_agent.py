...
class Heuristic_Agent(RL_Agent):
    ...
    def act(self, observations, num_obs=1,extra_info=False):
        ...
        assert explore_action.shape[0] == num_obs, f"The explorer heuristic functions does not returns the correct number of actions (batch dim) expected: {num_obs}, got: {explore_action.shape[0]}"
    ...
    def best_act_discrete(self, observations, num_obs=1, extra_info=False):
        return self.act(observations, num_obs=num_obs, extra_info=extra_info)
