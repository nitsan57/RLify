...
class TDQN_Agent(RL_Agent):
    """
    DQN Agent
    """
    def __init__(self, dqn_reg: float=0.0, batch_size: int=64, soft_exploit = False, explorer: Explorer = RandomExplorer(), latent_space_size = 100,  **kwargs):
        """
        Example::

            env_name = "CartPole-v1"
            env_c = gym.make(env_name, render_mode=None)
            agent_c = TDQN_Agent(obs_space=env_c.observation_space, action_space=env_c.action_space)
        ...
        super(TDQN_Agent, self).__init__(explorer=explorer, **kwargs, batch_size=batch_size)  # inits
        self.soft_exploit = soft_exploit

        assert self.n_actions == 1, "action space must be discrete with single action"
        
        self.q_table = torch.zeros(self.possible_actions, self.latent_space_size, device=self.device)    # Return a new array of given shape and type, filled with zeros

        self.dqn_reg = dqn_reg
        ### curr unused ### for soft target update future support
        self.tau = 0.001
        self.alpha = self.lr * 10
        ###################
    ...
    def init_models(self):
        ...
        if np.issubdtype(self.action_dtype, np.integer):
            self.encoder_model = self.model_class(input_shape=self.obs_shape, out_shape=self.latent_space_size, **self.model_kwargs).to(self.device)
            self.encode = lambda x, d: torch.softmax(self.encoder_model(x, d), -1)
            # self.target_Q_model = self.model_class(input_shape=self.obs_shape, out_shape=self.latent_space_size, **self.model_kwargs).to(self.device)
        
        with HiddenPrints():
            self.optimizer = adabelief_pytorch.AdaBelief(self.encoder_model.parameters(), self.lr, print_change_log=False, amsgrad=False)
    ...
    def save_agent(self,f_name) -> dict:
        ...
        save_dict['model'] = self.encoder_model.state_dict()
    ...
    def act_base(self, observations, num_obs=1,extra_info=False):
        ...
        batched_q_table_view = self.q_table.expand(num_obs, self.possible_actions, self.latent_space_size)
        with torch.no_grad():
            self.encoder_model.eval()
            encoded_table_states = self.encode(states, torch.ones((num_obs,1))).argmax(-1)
            batched_q_table_view[torch.arange(num_obs), :, encoded_table_states]
            all_actions = (batched_q_table_view[torch.arange(num_obs), :, encoded_table_states])
        self.encoder_model.train()
    ...
    def update_policy(self, *exp):
        ...
        for g in range(num_grad_updates):
            for b in range(0,all_samples_len, b_size):
                ...
                real_batch_size = batched_states.len   # only because last batch is smaller

                # v_table = self.Q_model(batched_states, batched_dones)

                batched_q_table_view = self.q_table.expand(real_batch_size, self.possible_actions, self.latent_space_size).clone().detach()
                relevant_q_actions_view = batched_q_table_view[torch.arange(real_batch_size), batched_actions.long()]
                states_latents_probs = self.encode(batched_states, batched_dones) # b,l shape
                q_values = (relevant_q_actions_view * states_latents_probs).sum(-1) # b,1 shape
                next_states_latents_probs = self.encode(batched_next_states, batched_dones)
                max_states_values = torch.max(batched_q_table_view, 1)[0]
                max_values = (max_states_values * next_states_latents_probs).sum(-1)
                

                new_q_values = (1-self.alpha)* q_values + self.alpha * (batched_rewards + self.discount_factor * max_values * (1-batched_terminated))
                batched_q_table_view[torch.arange(real_batch_size), batched_actions.long()] = ((1-states_latents_probs)*relevant_q_actions_view + states_latents_probs*new_q_values.unsqueeze(-1))

                self.q_table = batched_q_table_view.mean(0)
           
                loss = self.criterion(q_values, new_q_values) #+ self.dqn_reg*q_values.pow(2).mean()
